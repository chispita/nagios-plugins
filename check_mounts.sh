...
while read ENTRY ;do
	...
	if [ $? -ne 0 ];then
	    MSG="$MSG\nFailure to touch a file under $MP/tmpfile :( \n"
	    RC=$STATE_CRITICAL
	fi
...
if diff -u $TMP_MOUNTS_FILE $TMP_FSTAB_FILE >/tmp/mounts_fstab.diff;then
        echo -en "MOUNTS OK: All mount points are properly mounted \n"
        if [ -n "$EXCLUDED" ];then
	...
	#fi
        ...
        echo -en "MOUNT CRITICAL: /etc/fstab and /proc/mounts are not identical \n\n"
